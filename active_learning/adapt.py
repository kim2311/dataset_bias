...
def test(args, model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            x = pred_values[corr == 1]
            y = pred_values[corr == 0]

            corr_count += (x[x > thres]).shape[0]       
            incorr_count += (y[y > thres]).shape[0]

            print(corr_count, incorr_count)

            tmp_incorr_conf = torch.max(pred_values[corr == 0])
            if (tmp_incorr_conf > max_incorr_conf):
                max_incorr_conf = tmp_incorr_conf
    ...
    print('\nHighest value of incorrect confidence for all predictions: {}\n'.format(max_incorr_conf))
...
def main():
    ...
    # train_set = MNIST_M(root_img='/local/a/ksivaman/active-learning/data/mnist_m/mnist_m_train',
    #                                 root_label='/local/a/ksivaman/active-learning/data/mnist_m/mnist_m_train_labels.txt')
    # test_set = MNIST_M()


    train_set = MNISTM(root="data", train=True, download=True)
    test_set = MNISTM(root="data", train=False, download=True)

    # train_usps = USPS(root="data", train=True, download=True, transform=transforms.Compose([
    #                                             transforms.ToTensor(),
    #                                             transforms.Normalize((0.1307,), (0.3081,))
    #                                         ]))
    # test_usps = USPS(root="data", train=False, download=True, transform=transforms.Compose([
    #                                             transforms.ToTensor(),
    #                                             transforms.Normalize((0.1307,), (0.3081,))
    #                                         ]))

    train_set_orig_MNIST = datasets.MNIST('../data', train=True, download=True,
                                            transform=transforms.Compose([
                                                transforms.ToTensor(),
                                                transforms.Normalize((0.1307,), (0.3081,))
                                            ]))
    test_set_orig_MNIST = datasets.MNIST('../data', train=False, transform=transforms.Compose([
                                            transforms.ToTensor(),
                                            transforms.Normalize((0.1307,), (0.3081,))
                                        ]))
    ...
    if (args.save_model):
        torch.save(model.state_dict(),"models/mnist_m_cross_train_cnn.pt")
